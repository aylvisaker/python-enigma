...
def onKeyPress(event):
    ...
    if (x not in letters): x = ''
    plain = plain + x
    current = [x for x in initial]
    for p in plain:
        current = rotorstep(current)
    cipher = enigmaC([let[x] for x in initial], plain)
    a = current[0] - rings[0]
    b = current[1] - rings[1]
    c = current[2] - rings[2]
    text.delete(1.0,'end')
    text.insert('end',   'initial positions:    ' + ''.join([let[x] for x in initial]))
    text.insert('end', '\nring settings:        ' + ''.join(rin) + '\n')
    text.insert('end', '\n' + plain)
    text.insert('end', '\n' + cipher + '\n')
    text.insert('end', '\ncurrent position:     ' + ''.join([let[x] for x in current]))
    text.insert('end', '\nplaintext alphabet:   ' + letters)
    text.insert('end', '\nplugboard:            ' + ''.join([let[pb[x]] for x in range(26)]))
    text.insert('end', '\nright rotor:          ' + ''.join([let[(r2[(x + c)%26] - c)%26] for x in range(26)]))
    text.insert('end', '\nmiddle rotor:         ' + ''.join([let[(r1[(x + b)%26] - b)%26] for x in range(26)]))
    text.insert('end', '\nleft rotor:           ' + ''.join([let[(r0[(x + a)%26] - a)%26] for x in range(26)]))
    text.insert('end', '\nreflector:            ' + ''.join([let[r[x]] for x in range(26)]))
    text.insert('end', '\nleft rotor inverse:   ' + ''.join([let[(r0i[(x + a)%26] - a)%26] for x in range(26)]))
    text.insert('end', '\nmiddle rotor inverse: ' + ''.join([let[(r1i[(x + b)%26] - b)%26] for x in range(26)]))
    text.insert('end', '\nright rotor inverse:  ' + ''.join([let[(r2i[(x + c)%26] - c)%26] for x in range(26)]))
    text.insert('end', '\nplugboard:            ' + ''.join([let[pb[x]] for x in range(26)]))
